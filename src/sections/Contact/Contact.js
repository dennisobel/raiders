              ...
              contactContent.map((contact) => { 
                ...
                return (
                  <div className='info-item' key={contact.id}> 
                    <div className='icon'>{contact.icon}</div> {/* Contact Info icon */}
                    <div className="content">
                      <h3>{contact.title}</h3> {/* Contact Info title */}
                      {/* Map over the details array to display each item */}
                      {details.map((value, index) => {
                        return <p key={index}>{value}</p> /* Contact Info Text */
                      })}
                    </div>
                  </div>
                )
          ...
          <div className='contact-container'>
            ...
            <form method="post" className='contact-form' id="contactUs-Form">
              ...
              <div className='input-box'>
                <input type="text" name="name" className='box' id="name" value={this.state.name} onChange={e => this.setState({ name: e.target.value })} placeholder="name" required /> {/* Name Input Field */}
                <input type="email" name="email" className='box' id="email" value={this.state.email} onChange={e => this.setState({ email: e.target.value })}  placeholder="email" required /> {/* Email Input Field */}
              </div>
              <input type="text" name="subject" className='box' id="subject" value={this.state.subject} onChange={e => this.setState({ subject: e.target.value })} placeholder="subject" /> {/* Subject Input Field */}
              <textarea cols="30" rows="10" name="comment" id="comment" className='box' value={this.state.message} onChange={e => this.setState({ message: e.target.value })} placeholder="message"></textarea> {/* Textarea for message */}
              ...
              <div className='msg-alert'>
                ...
                {this.state.formError && (
                  <span className='alert'>Please fill in all fields.</span> /* Alert for form errors */
                )}

                {/* Mail Sent Alert */}
                {this.state.mailSent &&
                  <span className="alert">Message sent successfully</span> /* Alert for successful message sent */
                }
              </div>
